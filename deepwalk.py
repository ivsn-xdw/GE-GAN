...
class Graph_embedding(object):
    """Get graph embedding information"""
    def __init__(self, name, number_walks, walk_length,
                 window_size,
                 representation_size,
                 seed=0,
                 workers=1):
    ...
    def _call(self, inputs):
        G = load_adjacencylist(inputs, undirected=True)
        ...
        print("Training...")
        model = Word2Vec(walks, size=self.representation_size,
                         window=self.window_size,
                         min_count=0,
                         sg=1,
                         hs=1,
                         workers=self.workers)
        model.wv.save_word2vec_format(self.name)
